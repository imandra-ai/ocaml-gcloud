...
module rec Expression : sig
  ...
  type t
  type type_ = DATE | TIMESTAMP | INT64 | NUMERIC | STRING
  type direction
  ...
  and fn_info = { name : string; args : t list; fn_type : fn_type }
  and fn_type = Standard | Aggregate | Window
  and type_ = DATE | TIMESTAMP | INT64 | NUMERIC | STRING
  ...
  let rec apply ~f_e ?(f_q = fun q -> q) e =
    let f = f_e in
    match e with
    | Raw _ | Star | Identifier _ | Value _ | Param _ -> e
    | Array_lit es -> Array_lit (List.map f es)
    | Struct_lit_tuple es -> Struct_lit_tuple (List.map f es)
    | Struct_lit_typeless fs ->
        Struct_lit_typeless (List.map (fun (e, alias) -> (f e, alias)) fs)
    | Field (e, field) -> Field (f e, field)
    | Case (e, whens, else_) ->
        Case
          ( Util.Opt.map f e,
    ...
    | Not e -> Not (f e)
    | Is_null (e, b) -> Is_null (f e, b)
    | Equal (e1, e2) -> Equal (f e1, f e2)
    | Is_distinct_from (e1, e2) -> Is_distinct_from (f e1, f e2)
    | Is_not_distinct_from (e1, e2) -> Is_not_distinct_from (f e1, f e2)
    | Neq (e1, e2) -> Neq (f e1, f e2)
    | LT (e1, e2) -> LT (f e1, f e2)
    | LTE (e1, e2) -> LTE (f e1, f e2)
    | GT (e1, e2) -> GT (f e1, f e2)
    | GTE (e1, e2) -> GTE (f e1, f e2)
    | In (e1, e2) -> In (f e1, List.map f e2)
    | In_unnest (e1, e2) -> In_unnest (f e1, f e2)
    | And es -> And (List.map f es)
    | Or es -> Or (List.map f es)
    | Count e -> Count (match e with None -> None | Some e -> Some (f e))
    | Cast (e1, t) -> Cast (f e1, t)
    | Func v -> Func { v with args = List.map f v.args }
    | Array_agg a -> Array_agg (apply_array_agg ~f_e ~f_q a)
    | Over (e, window_specification) ->
        Over (f e, apply_window_specification ~f_e ~f_q window_specification)
    | Interval (e, date_part) -> Interval (f e, date_part)
    | Timestamp_diff (e1, e2, date_part) ->
        Timestamp_diff (f e1, f e2, date_part)
    | Timestamp_trunc (e, date_part) -> Timestamp_trunc (f e, date_part)
    | Time_diff (e1, e2, date_part) -> Time_diff (f e1, f e2, date_part)
    | Extract (date_part, e, at_time_zone) ->
        Extract (date_part, f e, at_time_zone)
    | Query q -> Query (f_q q)
    | Array q -> Array (f_q q)
    | Offset (e, i) -> Offset (f e, f i)
    | Exists q -> Exists (f_q q)
  ...
  let rec pp fmt =
    let open Format in
    function
    | Raw sql -> Format.fprintf fmt "%s" sql
    | Star -> Format.fprintf fmt "*"
    | Identifier i -> Format.fprintf fmt "%s" i
    | Value v -> Format.fprintf fmt "%s" (sql_of_value v)
    | Param param -> Format.fprintf fmt "@%s" param
    | Array_lit es ->
        fprintf fmt "@[<hv 1>[ %a@ ]@]"
    ...
    | Struct_lit_typeless fs ->
        let pp_as fmt (e, alias) =
          match alias with
          | None -> pp fmt e
          | Some alias -> fprintf fmt "@[<hv 1>%a@ AS %s@]" pp e alias
        in
        fprintf fmt "@[<hv 1>STRUCT(%a)@]"
          (pp_print_list ~pp_sep:(fun fmt () -> fprintf fmt ",@ ") pp_as)
          fs
    | Field (e, field) -> Format.fprintf fmt "%a.%s" pp_parens e field
    | Case (e, whens, else_) ->
        Format.fprintf fmt "CASE %a %a%a END" (Util.some pp) e
          ...
          whens
          (fun fmt -> function
            | Some else_ -> Format.fprintf fmt " ELSE %a" pp else_
            | None -> ())
          else_
    | Plus (e1, e2) -> Format.fprintf fmt "@[<hv 1>(%a@ + %a)@]" pp e1 pp e2
    | Minus (e1, e2) -> Format.fprintf fmt "@[<hv 1>(%a@ - %a)@]" pp e1 pp e2
    | Mul (e1, e2) -> Format.fprintf fmt "@[<hv 1>(%a@ * %a)@]" pp e1 pp e2
    | Div (e1, e2) -> Format.fprintf fmt "@[<hv 1>(%a@ / %a)@]" pp e1 pp e2
    | Not e -> Format.fprintf fmt "NOT %a" pp e
    | Is_null (e, b) ->
        Format.fprintf fmt "%a IS%s NULL" pp e (if b then "" else " NOT")
    | Equal (e1, e2) -> Format.fprintf fmt "%a = %a" pp_parens e1 pp_parens e2
    ...
    | Neq (e1, e2) -> Format.fprintf fmt "%a <> %a" pp e1 pp e2
    | LT (e1, e2) -> Format.fprintf fmt "%a < %a" pp e1 pp e2
    | LTE (e1, e2) -> Format.fprintf fmt "%a <= %a" pp e1 pp e2
    | GT (e1, e2) -> Format.fprintf fmt "%a > %a" pp e1 pp e2
    | GTE (e1, e2) -> Format.fprintf fmt "%a >= %a" pp e1 pp e2
    | In (e1, e2) ->
        Format.fprintf fmt "%a IN (%a)" pp e1
          (Format.pp_print_list
             ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ")
             pp)
          e2
    | In_unnest (e1, e2) -> Format.fprintf fmt "%a IN UNNEST(%a)" pp e1 pp e2
    | And es ->
        Format.(
          fprintf fmt "@[<hv 1>(%a)@]"
    ...
    | Count e ->
        Format.(
          fprintf fmt "COUNT(%a)"
            (fun fmt e ->
              match e with
              | Some e -> fprintf fmt "DISTINCT %a" pp e
              | None -> fprintf fmt " * ")
            e)
    | Cast (e1, t) -> Format.fprintf fmt "CAST(%a AS %a)" pp e1 pp_type t
    | Func { name; args; _ } ->
    ...
    | Array_agg a ->
        ...
        (match a.limit with
        | None -> ()
        | Some limit -> f "@ LIMIT %a" pp limit);
        f ")@]"
    | Over (e, window_specification) ->
        Format.fprintf fmt "@[<hv 2>%a OVER (@,%a)@]" pp e
          pp_window_specification window_specification
    | Interval (e, date_part) ->
        Format.fprintf fmt "INTERVAL %a %a" pp e pp_date_part date_part
    | Timestamp_diff (e1, e2, date_part) ->
        Format.fprintf fmt "@[<hv 2>TIMESTAMP_DIFF(@,%a,@ %a,@ %a)@]" pp e1 pp
          e2 pp_date_part date_part
    | Timestamp_trunc (e, date_part) ->
        Format.fprintf fmt "@[<hv 2>TIMESTAMP_TRUNC(@,%a,@ %a)@]" pp e
          pp_date_part date_part
    | Time_diff (e1, e2, date_part) ->
        Format.fprintf fmt "@[<hv 2>TIME_DIFF(@,%a,@ %a,@ %a)@]" pp e1 pp e2
          pp_date_part date_part
    | Extract (date_part, e, at_time_zone) ->
        Format.fprintf fmt "EXTRACT(%a FROM %a%a)" pp_date_part date_part pp e
          (Util.some (fun fmt tz -> Format.fprintf fmt " AT TIME ZONE %S" tz))
          at_time_zone
    | Query q -> Format.fprintf fmt "@[<hv 1>(%a)@]" Query_expr.pp q
    | Array q -> Format.fprintf fmt "@[<hv 2>ARRAY(@,%a@])" Query_expr.pp q
    | Offset (e, i) -> Format.fprintf fmt "%a[OFFSET(%a)]" pp_parens e pp i
    | Exists q -> Format.fprintf fmt "@[<hv 2>EXISTS (@,%a@])" Query_expr.pp q

  and pp_parens fmt t =
    let open Format in
    match t with
    | Identifier _ | Star | Field _ | Param _ -> pp fmt t
  ...
  let rec prefix_ident prefix = function
    | Identifier i -> Identifier (prefix ^ "." ^ i)
    | e -> apply ~f_e:(prefix_ident prefix) e

  (** Returns true if any of the functions called in the expression is an aggreate function. *)
  let rec is_aggregate = function
    | Raw _ | Star | Identifier _ | Value _ | Param _ -> false
    | Array_lit es -> List.exists is_aggregate es
    | Struct_lit_tuple es -> List.exists is_aggregate es
    | Struct_lit_typeless fs ->
    ...
    | Plus (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Minus (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Mul (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Div (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Not e -> is_aggregate e
    | Is_null (e, _) -> is_aggregate e
    | Equal (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Is_distinct_from (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Is_not_distinct_from (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | Neq (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | LT (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | LTE (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | GT (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | GTE (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | In (e1, e2) -> is_aggregate e1 || List.exists is_aggregate e2
    | In_unnest (e1, e2) -> is_aggregate e1 || is_aggregate e2
    | And es -> List.exists is_aggregate es
    | Or es -> List.exists is_aggregate es
    | Count _ -> true
    | Cast (e1, _) -> is_aggregate e1
    | Func { fn_type = Aggregate; _ } -> true
    | Func v -> List.exists is_aggregate v.args
    | Array_agg _ -> true
    | Over (e, _window_specification) -> is_aggregate e
    | Interval (e, _date_part) -> is_aggregate e
    | Timestamp_diff (e1, e2, _) -> is_aggregate e1 || is_aggregate e2
    | Timestamp_trunc (e, _) -> is_aggregate e
    | Time_diff (e1, e2, _) -> is_aggregate e1 || is_aggregate e2
    | Extract (_, e, _) -> is_aggregate e
    | Query _ -> false
    | Array _ -> false
    | Offset (e, _) -> is_aggregate e
    | Exists _ -> false
  ...
  let ( && ) e1 e2 =
    match (e1, e2) with
    | And e1s, And e2s -> And (e1s @ e2s)
    | And e1s, e2 -> And (e1s @ [ e2 ])
    | e1, And e2s -> And (e1 :: e2s)
    | e1, e2 -> And [ e1; e2 ]

  let ands = function e :: es -> List.fold_left ( && ) e es | [] -> bool true

  let ands_opt = function
    | e :: es -> Some (List.fold_left ( && ) e es)
    | [] -> None

  let ( || ) e1 e2 =
    match (e1, e2) with
    | Or e1s, Or e2s -> Or (e1s @ e2s)
    | Or e1s, e2 -> Or (e1s @ [ e2 ])
    | e1, Or e2s -> Or (e1 :: e2s)
    | e1, e2 -> Or [ e1; e2 ]
  ...
  let array_agg ?(distinct = false) ?(ignore_nulls = false)
      ?(respect_nulls = false) ?order_by ?limit e =
    Array_agg
      { distinct; ignore_nulls; respect_nulls; order_by; limit; expression = e }

  let window_specification ?named_window ?partition_by ?order_by
      ?window_frame_clause () =
  ...
  let round ?places e =
    make_fn "ROUND" (match places with Some x -> [ e; x ] | None -> [ e ])
...
and Query_expr : sig
  ...
  type from_item
  type join
  type join_type = INNER | CROSS | FULL | LEFT | RIGHT
  type named_window
  ...
  and select = Select of select_body | Union_all of select_body list
  ...
  and join_type = INNER | CROSS | FULL | LEFT | RIGHT
  ...
  let with_aliased ~as_ from_item =
    ...
    | From_ident (table_name, _) -> From_ident (table_name, Some as_)
    | From_query_expr (t, _) -> From_query_expr (t, Some as_)
    | From_join join -> From_join join
    ...
    Format.(
      fprintf fmt "%a%a%a%a"
        (Util.pp_non_empty_list (fun fmt withs ->
             fprintf fmt "@[<2>WITH@ %a@]@ "
               (Util.pp_comma_sep_list pp_with)
               withs))
        t.with_ pp_select t.select
  ...
  and pp_limit fmt limit =
    Format.(
      fprintf fmt "@ LIMIT %a%a" Expression.pp limit.count (Util.some pp_offset)
        limit.offset)
  ...
  and pp_select fmt select =
    match select with
    | Select r -> Format.(fprintf fmt "%a" pp_select_body r)
    | Union_all ts ->
        Format.(
          fprintf fmt "%a"
            (Format.pp_print_list
  ...
  and pp_select_item fmt { expression; alias; except } =
    Format.(
      let pp_except fmt names =
        fprintf fmt "@ @[<hv 2>EXCEPT (@,%a)@]"
          (Util.pp_comma_sep_list Util.pp_string)
          names
      in
      fprintf fmt "%a%a%a" Expression.pp expression
        (Util.pp_non_empty_list pp_except)
        except (Util.some pp_alias) alias)
  ...
  and pp_from_item fmt =
    Format.(
      ...
      | From_join join -> pp_join fmt join
      | From_unnest (expression, alias, with_offset_as) ->
          fprintf fmt "@[<hv 2>UNNEST(@,%a)%a%a@]" Expression.pp expression
            (Util.some pp_alias) alias
            (Util.some (fun fmt as_ -> fprintf fmt " WITH OFFSET AS %s" as_))
            with_offset_as)

  and pp_join fmt join =
    Format.(
      fprintf fmt "@[<hv>%a@ %aJOIN@ %a%a%a@]" pp_from_item join.left
        (Util.some pp_join_type) join.type_ pp_from_item join.right
        (Util.some (fun fmt on -> fprintf fmt "@ ON %a" Expression.pp on))
        join.on
        (Util.pp_non_empty_list (fun fmt es ->
             fprintf fmt "@ @[<hv 2>USING (%a)@]"
               (Util.pp_comma_sep_list Expression.pp)
               es))
        join.using)
  ...
  let ident ?prefix ?as_ name =
    match prefix with
    | None -> From_ident (name, as_)
    | Some p -> From_ident (p ^ "." ^ name, as_)
  ...
  let named_window ?named_window ?partition_by ?order_by ?window_frame_clause
      name =
    {
      name;
      window_specification =
        Expression.window_specification ?named_window ?partition_by ?order_by
          ?window_frame_clause ();
    }

  let select ?(distinct = false) ?(as_struct = false) ?from ?where
      ?(group_by = []) ?having ?(window = []) expressions =
    Select
      {
        distinct;
        as_struct;
  ...
  let rec apply ~(f_e : Expression.t -> Expression.t) ~(f_q : t -> t) (t : t) :
    ...
    let rec ae_select = function
      | Select b -> Select (ae_select_body b)
      | Union_all bs -> Union_all (List.map ae_select_body bs)
    ...
    and ae_from_item = function
      | From_ident (a, b) -> From_ident (a, b)
      | From_query_expr (e, s) -> From_query_expr (apply ~f_e ~f_q e, s)
      | From_join join -> From_join (ae_join join)
      | From_unnest (e, s1, s2) -> From_unnest (apply_e e, s1, s2)
    and ae_join x =
      {

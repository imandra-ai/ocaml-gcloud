...
module FirewallRules = struct
  ...
  let insert ~(project : string) ~(rule : rule) : (string, [> Error.t]) Lwt_result.t =
...
Auth.get_access_token ~scopes:[Scopes.cloud_platform; Scopes.compute] ()
    ...
    Lwt.catch
      (fun () ->
         let uri = Uri.make ()
             ...
             ~path:(Printf.sprintf "compute/v1/projects/%s/global/firewalls" project)
         in
         let headers =
         ...
         (Cohttp_lwt_unix.Client.post uri ~body ~headers:headers)
         |> Lwt_result.ok
      )
  ...
  let delete ~(project : string) ~(name : string) : (string, [> Error.t]) Lwt_result.t =
    let open Lwt_result.Infix in
    ...
    Lwt.catch
      (fun () ->
         let uri = Uri.make ()
             ~scheme:"https"
             ~host:"www.googleapis.com"
             ~path:(Printf.sprintf "compute/v1/projects/%s/global/firewalls/%s" project name)
         in
         let headers =
           Cohttp.Header.of_list
             [ "Authorization", Printf.sprintf "Bearer %s" token_info.Auth.token.access_token
         ...
         in
         (Cohttp_lwt_unix.Client.delete uri ~headers:headers)
         |> Lwt_result.ok
      )
      (fun e -> Lwt_result.fail (`Network_error e))
    >>= fun (resp, body) ->

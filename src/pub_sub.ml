...
module Subscriptions = struct
  ...
  let pull ?project_id ~subscription_id ~max_messages ?(return_immediately=true) () =
    ...
    let request = { maxMessages = max_messages; returnImmediately = return_immediately } in

    Lwt.catch (fun () ->
        let uri = Uri.make ()
            ~scheme:"https"
            ~host:"pubsub.googleapis.com"
            ~path:(Printf.sprintf "/v1/projects/%s/subscriptions/%s:pull" project_id subscription_id)

        in
        let body_str =
          request
          |> pull_request_to_yojson
          |> Yojson.Safe.to_string
        in
        let body =
          body_str
